...
def load(filename='../data/yahoo-movie-reviews.json'):
    with open('../data/yahoo-movie-reviews.json', 'rb') as f:
        data = pd.read_json(f)
...
def make_polar(data, balance=True):
    ...
    data_polar = data_polar.loc[data_polar.rating != 3].copy()
    data_polar.rating.loc[data_polar.rating <= 2] = 1
    data_polar.rating.loc[data_polar.rating >= 4] = 5
    if balance:
